...
fromBlock _ _ _ =
    let
        -- Thanks http://rbwhitaker.wikidot.com/index-and-vertex-buffers !
        v0 =
    ...
    { faces =
        List.map
            (\( x, y, z ) ->
                { vertices = [ z, y, x ]
                , normal = Vec3.cross (Vec3.sub y z) (Vec3.sub y x)
                }
            )
            faces
    , vertices = [ v0, v1, v2, v3, v4, v5, v6, v7, v8, v9, v10, v11 ]
    , uniqueEdges = [] -- Maybe we don't need this? It's tough...
    , uniqueNormals = List.map (\( x, y, z ) -> Vec3.cross (Vec3.sub y z) (Vec3.sub y x)) faces
    , volume = 0 -- Maybe we don't need this?
    , position = Vec3.zero
    }


fromunBlock : Float -> Float -> Float -> Convex
fromunBlock x y z =
    let
        v0 =
            { x = -x, y = -y, z = -z }
